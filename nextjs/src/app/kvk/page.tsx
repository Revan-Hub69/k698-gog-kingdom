                      ...
                      return (
                        <div key={row.id} style={{
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 7 }}>
                            ...
                            <span style={{ fontSize: 13, fontWeight: 600, color: '#fff', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.name}</span>
                            <div style={{ display: 'flex', gap: 6, flexShrink: 0, alignItems: 'center' }}>
                              {row.score && <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>Pt. {fmt(row.score)}</span>}
                              {row.deaths && <span style={{ fontSize: 11, color: 'rgba(248,113,113,0.5)', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>💀 {fmt(row.deaths)}</span>}
                              <span style={{ fontSize: 11, fontWeight: 800, color: total >= 3 ? '#4ade80' : total > 0 ? '#fbbf24' : 'rgba(255,255,255,0.2)', minWidth: 22, textAlign: 'right' }}>{total}/3</span>
                            </div>
                          </div>
                          {/* ROW 2: pack controls */}
                          <div style={{ display: 'flex', gap: 6 }}>
                            {([['p90','90',PC['90']],['p60','60',PC['60']],['p30','30',PC['30']]] as const).map(([k, label, color]) => (
                              <div key={k} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 3, background: `${color}10`, borderRadius: 8, padding: '4px 6px', border: `1px solid ${color}${row[k] > 0 ? '40' : '18'}` }}>
                                <button onClick={() => changePack(row.id, k, -1)} style={{ width: 24, height: 24, borderRadius: 5, border: 'none', background: row[k] > 0 ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.04)', color: row[k] > 0 ? '#fff' : 'rgba(255,255,255,0.2)', cursor: row[k] > 0 ? 'pointer' : 'default', fontSize: 15, lineHeight: 1 }}>−</button>
                                <span style={{ flex: 1, textAlign: 'center', fontSize: 13, fontWeight: 900, color: row[k] > 0 ? color : 'rgba(255,255,255,0.2)' }}>{row[k]}</span>
                                <span style={{ fontSize: 10, fontWeight: 700, color: row[k] > 0 ? color : 'rgba(255,255,255,0.2)' }}>×{label}</span>
                                <button onClick={() => changePack(row.id, k, +1)} disabled={total >= 3} style={{ width: 24, height: 24, borderRadius: 5, border: 'none', background: total >= 3 ? 'rgba(255,255,255,0.03)' : `${color}25`, color: total >= 3 ? 'rgba(255,255,255,0.1)' : color, cursor: total >= 3 ? 'not-allowed' : 'pointer', fontSize: 15, lineHeight: 1 }}>+</button>
                              </div>
                            ))}
                          </div>
                        </div>
